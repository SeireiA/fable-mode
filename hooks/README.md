...
This directory supports the original Claude Code hooks and a Codex-compatible
installation. For Codex-specific setup and limitations, see
`../README.codex.zh-CN.md` and run `py -3 ../install_codex.py`.
...
| `fable_spawn_guard.py` | Claude: `PreToolUse`; Codex: `SubagentStart` | Claude can block; Codex injects a design-gate warning because `SubagentStart` cannot cancel a built-in subagent. |
...
The four hooks are registered in the global client configuration and fire for every
...
defaults to the conservative tier.
...
## Install / register for Codex

Install the repository as `~/.codex/skills/fable-mode`, then run:

```powershell
py -3 "$HOME/.codex/skills/fable-mode/install_codex.py"
```

Codex CLI and Codex desktop share this configuration. Restart both clients and
use `/hooks` to review and trust the exact hook commands. See
`README.codex.zh-CN.md` for the event mapping and the current subagent limitation.

## Install / register for Claude Code
