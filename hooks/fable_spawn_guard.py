...
Codex currently does not expose built-in subagent creation to PreToolUse.
When installed as a Codex SubagentStart hook, this script injects a design-gate
warning into a subagent that starts without an open ledger card. It cannot
...
def codex_subagent_start(data):
    """Inject Codex-compatible context when delegation skipped the plan gate.

    SubagentStart has no task payload and cannot block startup in current Codex
    releases. An open card is therefore the only reliable signal available.
    """
    ...
    print(json.dumps({
        "systemMessage": "fable-mode: delegation started without an open card",
        "hookSpecificOutput": {
