...
def routing_text(routing):
    ...
    return head + (
        " Codex built-in subagents inherit the session model because "
        "spawn_agent currently has no model selector; in that environment, "
        "inherit regardless of profile and never claim a downgrade occurred."
...
def build_context(profile, model, ledger_state, open_items, routing):
    ...
    if ledger_state == "idle":
        return (
            ...
            "fan-out should have a live open card). Always: audit every progress "
            "claim against a tool result; don't end the turn on an actionable "
            "promise; lead with the outcome. Codex subagents inherit the "
            "session model unless the active tool explicitly exposes a "
    ...
    lines = [
        ...
        "Codex warns subagents started without OPEN cards; the Stop guard "
        "blocks stopping with open cards or checking a card `- [x]` without "
        "a substantive `-- evidence:` note.",
