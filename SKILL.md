---
name: fable-mode
description: A Fable-5-inspired work-discipline protocol adapted for Codex CLI and Codex desktop. It uses a plan gate, small-card execution, adversarial self-check, real-product verification, context hygiene, and checkpoint autonomy. Activate ONLY on an explicit request — the user names the mode ("use fable mode", "enable fable-mode", "work like Fable 5", "rigorous mode", or the Chinese "用 fable 模式"、"开 fable-mode"、"像 Fable 5 一样做"、"严谨模式") — or when the project is already armed with a `.fable/` directory. Do not auto-trigger merely because a task is large, important, or quality-sensitive.
triggers:
  - "fable mode"
  - "fable-mode"
  - "use fable mode"
...
- **Tool-reliability red line**: network fetches can hang without a timeout. Bound long-running fetches and give long background work a watchdog.
...
- **throughput**: use the concurrency the current Codex runtime exposes. Opens **only** when the user asks (see one-word controls); state the token and rate-limit cost. Never open it silently.
...
Codex built-in subagents currently inherit the session model because `spawn_agent` exposes no model selector. In that environment, all three profiles inherit; apply downgrade guidance only to an external executor or a future Codex surface that explicitly supports model selection.
...
- Acceptance fails once → retry with the failure output; fails twice → pull the card back inline on the session model. Do not claim a model override that the active Codex surface does not expose.
...
**Per-task granularity**: *active* (open cards) = full enforcement; *idle* (no/all-closed cards) = close guard quiet and small tasks flow freely; *paused* (a `PAUSED: reason` line) = guards off. Write PAUSED only when the user steers to work unrelated to the round; remove it to resume. Everything fails open (a guard bug never bricks the session); the Stop hook is loop-safe.
